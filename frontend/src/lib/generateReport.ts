...
/** Shared builder — returns the jsPDF doc so callers can save or export as needed. */
async function buildPDFDoc(
  ...
  if (briefing?.executive_summary && !briefing.executive_summary.includes("RESOURCE_EXHAUSTED")) {
    ...
    if (briefing.remediation_steps.length > 0) {
      ...
      briefing.remediation_steps.forEach((step, i) => { para(`${i + 1}. ${step}`); });
    }
  }
  ...
  result.anomalies.slice(0, topN).forEach((a, idx) => {
    newPageIfNeeded(8);
    if (idx % 2 === 0) { doc.setFillColor(250, 249, 252); doc.rect(M, y - 1, CW, 6.5, "F"); }
    doc.setFont("helvetica", "normal");
    doc.setFontSize(7.5);
    ...
    doc.text(a.parsed_log.timestamp ? a.parsed_log.timestamp.slice(0, 19) : "—", cols[4].x + 1, y + 3.5);
    y += 6.5;
  });
...
export async function generatePDFBase64(
  ...
  const doc = await buildPDFDoc(result, briefing, sourceFilename);
